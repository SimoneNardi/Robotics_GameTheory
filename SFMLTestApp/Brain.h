...
public:
	///
	virtual void update(Identifier _windows_id, sf::Event _event) = 0;

	///
	virtual bool guardGoalAchieved(std::shared_ptr<Player> _guard) = 0;

	///
	virtual bool thiefGoalAchieved(std::shared_ptr<Player> _thief) = 0;

	///
	virtual bool goalAchieved() = 0;
